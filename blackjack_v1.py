...
#-------------------------------------------------------------------------------#
def dealer_busts(hand_dealer):
    ''' check for dealer bust/(nothing to do if so)...except return True/False'''
#-------------------------------------------------------------------------------#   
    # if dealer exceeds 21...busted
    if (hand_dealer.value1 > 21 and hand_dealer.value11 > 21):
        return True
    else:
        return False

#-------------------------------------------------------------------------------#
def dealer_wins(hand_player, hand_dealer):
#-------------------------------------------------------------------------------#
    ''' check for dealer win/(nothing to do if so, besides...) return True/False'''
    # yeah, but what if dealer can still play some more? maybe has 14 and wants to go again?
    #  he cant 'win' more than once!
    # player_value? 
    player_value = hi_two_cons(hand_player.value1, hand_player.value11)    
    # dealer_value? 
